...
events/{eventId}                 # 予定（属するカレンダーの参加者のみ読書き可）
  - title, creatorId, participantIds[], calendarId, startAt, endAt, allDay,
    type, priority, memo, reminderOffsets{uid: [分]}, updatedBy,
...
- 表示対象カレンダーの既定は「自分が参加しているカレンダーの先頭（表示順）」。ユーザーの選択は端末ローカル（`SharedPreferences`）に保持し、参加していないカレンダーが選ばれている場合は先頭へフォールバックする。
- 表示対象は**複数選べる**（Issue #170）。選んだカレンダーの予定を月表示・日別一覧に重ねて描く。選択集合は端末ローカルに `calendars.visible_ids` として保存し、再起動後も維持する（旧・単一 ID の `calendars.selected_id` からは初回起動時に移行する）。同時表示の上限は 5 件で、切替 UI が上限超過を防ぐ。
- 重ね表示でも予定の色は「誰の予定か」（FR-2）を表す。カレンダーの違いは日別一覧のカレンダー名ラベルで示す。月表示の帯には**別カレンダーの予定の目印を付けない**（Issue #170 で一度実装したが削除）。理由は、色（FR-2）・枠線（FR-3）を既に使っている帯に第三の軸を足す納得できるデザインが思いつかなかったこと、および表示対象カレンダーは各自が端末ローカルに個別設定する項目であり、共有される見え方ではないため、目印は不要と判断したこと。
- 予定の新規作成時の既定カレンダーは、表示中カレンダー集合の先頭（＝表示順の先頭）。
...
| priority | number | 表示上の優先度。**1（最重要）〜10**、既定 5。月表示のレーン配置と日別一覧の並びで、値が小さい予定を先に置く（FR-1 / FR-4、Issue #176）。導入前のドキュメントにはキーが無いため**欠損時は 5 として読む**（移行は不要）。Security Rules は存在する場合のみ範囲を検証する |
| memo | string | メモ（任意） |
...
- `priority` はクエリ・インデックスの対象にしない。月表示は 6 週分を、日別一覧はその日ぶんを
  取得してから**クライアント側で並べ替える**ため、`startAt` の範囲クエリだけで足りる
  （複合インデックスの追加は不要）。
- 月表示は `deleted` / `calendarId` の一致 ＋ `startAt` の範囲クエリで取得する（複合インデックス
  `(deleted, calendarId, startAt)` を定義）。`calendarId` は必ず実際の `where` 句として
...
- 複数カレンダーの重ね表示（Issue #170）は、この**等値クエリをカレンダーごとに 1 本ずつ**
  張って結果を画面側で合成する。`whereIn` 1 本にまとめないのは上記の証明可能性を保つため
  （等値の `where` 句なら従来どおりルールが静的に成立する）。同じ複合インデックスを
  そのまま使うので、**インデックスの追加は不要**。
...
- **公開経路**: 外部に見せる URL は Cloudflare Worker（`cloudflare/api-proxy/`）の
  `https://kansuke-api.dreamyard.cc` 1 本に絞り、Cloud Functions の URL とプロジェクト ID を表に出さない。
...
- 表示順（FR-4）：月表示のレーン配置・日別一覧とも **「自分が参加する予定 → 優先度（1 が最重要） → 終日 → 開始日時」**。優先度を「自分の予定」の下に置くのは、閲覧者本人の予定が「+N」に隠れないという既存の保証を崩さないため。既定値（5）同士では従来どおりの並びになる。
- 優先度の表示：**既定値（5）から動かした予定にだけ**目印を出す。日別一覧はバッジ（重要度を上げた予定は目を引く色、下げた予定は控えめな色）、月表示の帯はタイトル先頭に `[n]` を付す。ほとんどの予定は既定のままなので、常に出すと目印がノイズになり「わざわざ動かした予定」が埋もれるため。
