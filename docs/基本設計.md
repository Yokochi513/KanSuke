...
- **FR-8（複数カレンダー機能）の既定カレンダー自動生成は、Blocking Function ではなくクライアント側の初期化処理で行う。** `beforeUserCreated` は新規アカウント作成時にしか発火せず、既にサインアップ済みの家族メンバーを事後的に既定カレンダーへ追加する手段にならないため。サインイン確定後にアプリが毎回冪等に `calendars/default`（固定 ID、`Calendar.create` の UUID ではなく `defaultCalendarId`）の存在を保証し、その時点で判明している家族メンバー全員を含める（詳細は §3.2）。
...
calendars/{calendarId}            # カレンダー（FR-8）。既定カレンダーのみ固定 ID 'default'
...
| フィールド | 型 | 説明 |
| --- | --- | --- |
| id | string | ドキュメント ID。既定カレンダー（わが家）のみ固定値 `'default'`、それ以外は**クライアント生成 UUID** |
| name | string | カレンダー名 |
| memberIds | string[] | 参加者（`users` の uid の配列、**1人以上必須**、allowlist 済み家族メンバーの中から選択）。このカレンダーの予定を閲覧・編集できる範囲 |
| creatorId | string | 作成者（`users` の uid） |
| createdAt / updatedAt | timestamp | 監査用 |

- 既定カレンダー（`calendars/default`）はサインイン確定後のクライアント初期化処理が自動生成し、その時点で判明している家族メンバー全員を `memberIds` に含める（§2.1）。以後、新しい家族メンバーが加わった場合もそのメンバーの初回起動時に自身を追加する。
- Security Rules 上、通常のカレンダーは `memberIds` に自分が含まれる場合のみ read/write 可。既定カレンダーのみ、read は家族全員に許可し、update は「自分を追加する」操作に限り非メンバーにも許可する（初回参加のため）。
...
| 予定編集 | タイトル/日時/終日/参加者（複数選択・必須）/種別（仮↔確定トグル）/メモ/リマインド設定 | FR-1, FR-3, FR-5 |
