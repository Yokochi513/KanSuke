...
- **Auth Blocking Function（`beforeUserCreated`）** はアカウント作成時に一度だけ発火し、初期データを作る：
  ...
  - `calendars/{uuid}`：**本人だけが参加する個人カレンダー**（`memberIds = [uid]`、名前は「〇〇のカレンダー」）。これが最初の表示対象になる（FR-8）。
- クライアントは認証完了後に `users/{uid}` の存在だけを確認し、無ければ初期化失敗としてサインアウトして再試行を促す。
- 権限分離は設けず、全メンバー同等権限（要件 §3）。予定の表示色判別は `events.participantIds` で行う。
...
calendars/{calendarId}            # カレンダー（FR-8）。ID は UUID（旧・既定カレンダーのみ 'default'）
  - name, memberIds[], creatorId, createdAt, updatedAt
...
| name | string | カレンダー名 |
| memberIds | string[] | 参加者（`users` の uid の配列、**1人以上必須**、サインアップ済みの利用者から選択）。このカレンダーの予定を閲覧・編集できる範囲 |
| creatorId | string | 作成者（`users` の uid） |
| createdAt / updatedAt | timestamp | 監査用 |

- **個人カレンダー**（`memberIds` が本人のみ）は、アカウント作成時に Auth Blocking Function が生成する（§2.1）。これが最初の表示対象になる。
- 表示対象カレンダーの既定は「自分が参加しているカレンダーの先頭（名前昇順）」。ユーザーの選択はアプリ内で保持し、参加していないカレンダーが選ばれている場合は先頭へフォールバックする。
- Security Rules 上、カレンダーは `memberIds` に自分が含まれる場合のみ read/write 可（旧・既定カレンダーへの特例参加は廃止、Issue #87）。
