...
events/{eventId}                 # 予定（家族で共有）
  - title, ownerId, startAt, endAt, allDay,
...
| title | string | タイトル |
| ownerId | string | 所有者（`users` の uid）。FR-2 |
...
| 予定編集 | タイトル/日時/終日/所有者/種別（仮↔確定トグル）/メモ/リマインド設定 | FR-1, FR-3, FR-5 |
...
- 所有者識別：`users.color` を予定バー/ドットに適用。凡例を表示。
